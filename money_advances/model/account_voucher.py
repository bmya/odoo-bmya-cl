...
class account_voucher(osv.Model):
    ...
    def writeoff_move_line_get(self, cr, uid, voucher_id, line_total, move_id, name, company_currency, current_currency, context=None):
        ...
        if move_line and not voucher.payment_option == 'with_writeoff' and voucher.partner_id:
            if voucher.type in ('sale', 'receipt'):
                ...
                account_id = voucher.advance_account_id and voucher.advance_account_id.id or voucher.partner_id.property_account_payable.id
            if not account_id:
    ...
    def onchange_partner_id(self, cr, uid, ids, partner_id, journal_id, amount, currency_id, ttype, date, context=None):
        ...
        if ttype in ('sale', 'receipt'):
            advance_account_id = partner.property_account_receivable and partner.property_account_receivable.id or False
        else:
            advance_account_id = partner.property_account_payable and partner.property_account_payable.id or False
