...
class AccountMove(models.Model):
    ...
    def check_match(self):
        for record in self:
            ...
            for line in record.line_ids.filtered(lambda s: s.purchase_line_id):
                ...
                # otherwise show a warning alert
                if line.quantity <= line.purchase_line_id.qty_received:
    ...
    def _onchange_purchase_auto_complete(self):
        ...
        self.fiscal_position_id = self.purchase_id.fiscal_position_id if not self.fiscal_position_id else self.fiscal_position_id
        self.invoice_payment_term_id = self.purchase_id.payment_term_id
        self.currency_id = self.purchase_id.currency_id if not self.currency_id else self.currency_id

        invoice_lines = self.line_ids.filtered(lambda s: not s.purchase_line_id)
        po_lines = self.purchase_id.order_line - self.line_ids.filtered(lambda s: s.purchase_line_id).mapped('purchase_line_id')
